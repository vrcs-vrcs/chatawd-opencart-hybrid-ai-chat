<?php
class ControllerExtensionModuleChataiwdSearch extends Controller {
    ...
    public function index(array $args) {
        ...
        $this->load->model('extension/module/chataiwd');
    ...
    private function getPromotionDiscount() {
        ...
        $getSpecials = function($filter_data) {
            ...
            if (property_exists($model, 'getSpecials')) {
                $vissza = $model->getSpecials($filter_data);
            } elseif (property_exists($model, 'getProductSpecials')) {
                $vissza = $model->getProductSpecials($filter_data);
            } else {
                $vissza = [];
            }
            return $vissza;
    ...
    private function getPromotionFeatured() {
        ...
        $results = $this->model_extension_module_chataiwd->getModulesByCode('opencart.featured');
        if (!$results) {
            $results = $this->model_extension_module_chataiwd->getModulesByCode('featured');
        }
    ...
    private function getPromotionNewest() {
        ...
        $model = $this->model_catalog_product;
        if (property_exists($model, 'getLatest')) {
            $results = $model->getLatest($this->max);

        } elseif (property_exists($model, 'getLatestProducts')) {
            $results = $model->getLatestProducts($this->max);
    ...
    private function getPromotionPopular() {
        ...
        $model = $this->model_catalog_product;
        if (property_exists($model, 'getBestSeller')) {
            $results = $model->getBestSeller($this->max);

        } elseif (property_exists($model, 'getBestSellerProducts')) {
            $results = $model->getBestSellerProducts($this->max);
    ...
    private function getCategory() {
        $categories = $this->model_extension_module_chataiwd->getCategories();
    ...
    private function getManufacturer() {
        $manufacturers = $this->model_extension_module_chataiwd->getManufacturers();
    ...
    private function getShipping() {
        $data_storage = [];
        $shipping_methods = $this->model_extension_module_chataiwd->getShippingMethods();
        $informations = $this->model_catalog_information->getInformations();
        $shipping_lang = $this->getSearchTerms('shipping');

        if (!empty($shipping_methods)) {
            foreach ($shipping_methods as $method) {
                $firstQuote = reset($method['quote']); // reset() visszaadja az első elemet

                $title      = $firstQuote['title'];
                $text       = $firstQuote['text'];
                $method_id  = $firstQuote['code'];
    ...
    private function getPayment() {
        ...
        $payment_address = $this->model_extension_module_chataiwd->getPaymentAddress();
        $payment_methods = $this->model_extension_module_chataiwd->getPaymentMethods($payment_address);
        $informations   = $this->model_catalog_information->getInformations();
    ...
    private function getProductAttributesScore($product_id) {
        ...
        if (property_exists($model, 'getProductAttributes')) {
            $attributes = $model->getProductAttributes($product_id);
        } elseif (property_exists($model, 'getAttributes')) {
        ...
        if (property_exists($model, 'getOptions')) {
            $options = $model->getOptions($product_id);
        } elseif (property_exists($model, 'getProductOptions')) {
            $options = $model->getProductOptions($product_id);
        } else {
            $options = [];
        }
