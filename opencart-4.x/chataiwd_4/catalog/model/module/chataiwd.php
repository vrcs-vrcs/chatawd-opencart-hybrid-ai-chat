<?php
namespace Opencart\Catalog\Model\Extension\chataiwd\Module;
...
*/
            ...
            if (isset($method['quote']) && is_array($method['quote'])) {
                ...
                foreach ($method['quote'] as $quote) {
                    $formatted_methods[] = [
                        'code' => $quote['code'] ?? '',
                        'title' => $quote['title'] ?? '',
                        'cost' => isset($quote['cost']) ? (float)$quote['cost'] : 0.0,
                        'text' => $quote['text'] ?? ($quote['cost'] ?? 0.0),
                        'sort_order' => isset($method['sort_order']) ? (int)$method['sort_order'] : 0
                    ];
                }
            }
        }
    ...
    public function getPaymentMethods(array $address): array {
        ...
        foreach ($payment_methods as $method) {
            ...
            // Extension lekérdezése az oc_extension táblából
            $query = $this->db->query("SELECT extension FROM " . DB_PREFIX . "extension WHERE type = 'payment' AND code = '" . $this->db->escape($code) . "'");
            $extension = $query->row['extension'] ?? 'opencart'; // Alapértelmezett: opencart


            $language_path = "extension/" . $extension . "/payment/" . $code;

            $this->load->language($language_path);
            $title = $this->language->get('heading_title') ?: $method['code']; // Ha nincs heading_title, fallback a title-ra
