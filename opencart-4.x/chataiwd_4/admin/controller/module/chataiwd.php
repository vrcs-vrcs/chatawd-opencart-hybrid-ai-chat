 ...
 * 2025-04-24
 */
namespace Opencart\Admin\Controller\Extension\chataiwd\Module;
...
class chataiwd extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        $data['local_stats_url']    = $this->url->link($this->model_load . $this->method_separator . 'getLocalStats' , 'user_token=' . $this->session->data['user_token'], true); // 10mp-enként kosár és egyéb infók köldése vrcs-re
        $data['push_chunk_url']     = $this->url->link($this->model_load . $this->method_separator . 'pushTableChunk' , 'user_token=' . $this->session->data['user_token'], true); // 10mp-enként kosár és egyéb infók köldése vrcs-re
        $data['finalize_sync_url']  = $this->url->link($this->model_load . $this->method_separator . 'finalizeSync' , 'user_token=' . $this->session->data['user_token'], true); // 10mp-enként kosár és egyéb infók köldése vrcs-re
        $data['billing_url']        = $this->url->link($this->model_load . $this->method_separator . 'billing' , 'user_token=' . $this->session->data['user_token'], true); // 10mp-enként kosár és egyéb infók köldése vrcs-re
        $data['register_url']       = $this->url->link($this->model_load . $this->method_separator . 'register' , 'user_token=' . $this->session->data['user_token'], true); // 10mp-enként kosár és egyéb infók köldése vrcs-re
        $data['save_url']           = $this->url->link($this->model_load . $this->method_separator . 'save' , 'user_token=' . $this->session->data['user_token'], true); // 10mp-enként kosár és egyéb infók köldése vrcs-re
        ...
        $this->document->addStyle('../extension/chataiwd/admin/view/stylesheet/chataiwd.css');
    ...
    private function addEvent() {
        ...
        $this->registerOpenCartEvent('chataiwd_catalog', 'ChatAIWD Catalog Injector', $trigger, $action);


        $this->model_setting_event->deleteEventByCode('chataiwd_reward_trigger');
        $trigger = 'catalog/model/checkout/order' . $this->method_separator . 'addHistory/after';
        $action = $this->model_load . $this->method_separator . 'onOrderUpdate';
        $this->registerOpenCartEvent('chataiwd_reward_trigger', 'ChatAIWD Reward Processor', $trigger, $action);
    }

    private function registerOpenCartEvent($code, $description, $trigger, $action) {
        if (version_compare(VERSION, '4.0.1.0', '>=')) {
            ...
            // Régebbi OC verziók (pl 3.x)
            $this->model_setting_event->addEvent($code, $description, $trigger, $action, true, 99);
        }
    }
    ...
    public function install(): void {
        ...
        if (version_compare(VERSION,'4.0.1.0','>=')) {
            $data = [
                ...
                'action'      => $this->model_load.$this->method_separator.'eventViewCommonColumnLeftBefore',
                'status'      => true,
                'sort_order'  => 0
            ];
            $this->model_setting_event->addEvent($data);
        } else {
            $this->model_setting_event->addEvent('chataiwd','','admin/view/common/column_left/before',$this->model_load.$this->method_separator.'eventViewCommonColumnLeftBefore');
        }
    ...
    public function eventViewCommonColumnLeftBefore(&$route, &$data, &$code) {
        ...
        $data['menus'][] = [
            ...
            'name'     => $heading_title,
            'href'     => $this->url->link($this->model_load, 'user_token=' . $this->session->data['user_token']),
            'children' => []
        ];
    }
